...
* `mandel` owns a small render host that draws CPU Mandelbrot output into
  ...
  display path owns an SDL GPU device, claims the SDL canvas window, and
  clears the swapchain from the GPU paint handler.
