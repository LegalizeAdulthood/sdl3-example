...
* `mandel` owns a small render host that draws CPU Mandelbrot output into
  ...
  static shader assets generated by shadercross, creates a Mandelbrot
  compute pipeline and fullscreen triangle pipeline, writes iteration
  counts into an `R32_FLOAT` texture, and blits that texture from the
  GPU paint handler.
...
CPU/GPU presentation switch.  The GPU path dispatches a fixed HLSL
compute shader and displays the resulting iteration texture with a
fullscreen triangle.  Keep SDL window access through
...
Use `R32_FLOAT` for the current continuous iteration output.
...
convert iteration counts to color with the same `chroma.map` palette used
by the CPU path.
